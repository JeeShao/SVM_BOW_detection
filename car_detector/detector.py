...
def bow_features(img, extractor_bow, detector):
  # b = extractor_bow.compute(img, detector.detect(img))[0]
  # print (b)
...
def car_detector():
  ...
  for i in range(SAMPLES):
    ...
    # bow_kmeans_trainer.add(extract_sift(lPath(i), extract, detect))
  ...
  for i in range(SAMPLES):
    ...
    #
    # traindata.extend(bow_features(cv2.imread(lPath(i), 0), extract_bow, detect))
    # trainlabels.append(2)

    traindata.extend(bow_features(cv2.imread(path(neg, i), 0), extract_bow, detect)) #extend
    trainlabels.append(-1)
